...
const STOP_WORDS = new Set([
  ...
  "only", "self", "pickup", "latest", "item", "items"
]);

const PRODUCT_TERMS = new Set([
  "sofa", "chair", "table", "storage", "organizer", "kitchen", "dress", "pants", "jeans",
  "sandals", "slippers", "shoes", "bracelet", "necklace", "watch", "bag", "lamp", "toy",
  "makeup", "skincare", "phone", "car", "baby", "fitness", "bottle", "shower", "mat",
  "blanket", "jacket", "hoodie", "cap", "hat", "shelf", "rack", "pet", "dog", "cat",
  "ring", "pillow", "travel", "office", "home", "beauty", "bathroom", "bed", "garden",
  "outdoor", "airplane", "neck", "purse", "wallet", "case", "cover", "earrings", "blazer",
  "blazers", "body", "care", "party", "supplies", "tool", "tools", "cleanser", "spray",
  "serum", "cream", "facial", "shoe", "kids", "child", "children", "decor"
]);
...
export function extractProductKeywords(productOrName, categoryArg = "", maxKeywords = 8) {
  ...
  for (const phrase of basePhrases) {
    ...
    if (productWords.length >= 3) collected.push(productWords.slice(0, 3).join(" "));
    if (productWords.length >= 2) collected.push(productWords.slice(0, 2).join(" "));
    if (productWords.length === 1) collected.push(productWords[0]);

    // Keep the full cleaned phrase, but rank it lower because long CJ titles often have no Trends data.
  ...
  return unique(collected)
    .map(keyword => {
      ...
      return {
        keyword,
        score: importantCount * 5 + phraseBonus + longPenalty + oneWordPenalty + Math.min(3, words.length)
      };
    })
