...
// Strict mode: cache-level + picker-level meaningful overlap check.
...
// Words to ignore when calculating meaningful overlap – these are too generic.
...
async function fetchTrendForVariantCached(page, variant, originalKeyword) {
  ...
  // **Cache-level overlap check**: if the cached variant has zero meaningful overlap with the current product, reject it.
  const originalWords = new Set(cleanKeyword(originalKeyword).split(/\s+/).filter(w => !OVERLAP_IGNORE_WORDS.has(w)));
  if (originalWords.size > 0) {
    ...
    if (overlap === 0) {
      console.log(`Cache-level reject: variant "${variant}" has no meaningful overlap with "${originalKeyword}". Fetching fresh.`);
      VARIANT_RESULT_CACHE.delete(variant);
...
/**
 * Pick the best variant signal with strict meaningful overlap.
 * Only variants that share at least one non-generic word with the original keyword are considered.
...
function pickBestVariantSignal(signals, originalKeyword) {
  ...
  // If original has no meaningful words, we can't enforce overlap – pick highest score (rare case).
  if (originalWords.size === 0) {
  ...
  const scored = matched.map(signal => {
    ...
    for (const w of signalWords) {
      if (originalWords.has(w)) meaningfulOverlap += 1;
    }
  ...
  if (candidatesWithOverlap.length === 0) {
    console.log(`Google Trends rejecting all variants for "${originalKeyword}" – no meaningful overlap.`);
    return null;
  }
...
writeJson("google-trends-meta.json", {
  ...
  note: "Strict mode: cache-level + picker-level meaningful overlap check. Only variants sharing at least one non-generic word are accepted. Products without relevant variant get no Google score.",
