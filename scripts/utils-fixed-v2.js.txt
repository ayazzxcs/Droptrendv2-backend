...
export function extractKeywords(products, limit = 120) {
  ...
  products.forEach((product, index) => {
    // Keep every Google Trends keyword entry tied to exactly one product.
    // Do NOT group multiple products under the same keyword. Shared keywords
    // such as "adapter", "light", "bed", or "cover" can otherwise produce one
    // trend signal whose productIds list contains unrelated products. If a later
    // merge maps that one signal back to every productId, product pages show the
    // wrong trend phrase.
  ...
  // Product-specific entries should remain product-specific. Sorting is only for
  // selecting the most useful keyword jobs within the global limit; the output
  // objects themselves are never grouped across products.
  return entries
    .sort((a, b) =>
    ...
    .slice(0, limit)
    .map(({ anchorRank, productKey, ...entry }) => entry);
}
