...
writeJson("lens-amazon-meta.json", {
  ...
  note: "Monthly cache mode: first run of a new month starts fresh; later same-month runs skip existing Amazon data. Uses one Lens provider until its limit is reached, then moves to the next. For each candidate, it extracts Amazon DOM, JSON-LD and embedded structured metadata. When a page is unavailable, it may use rating/review metadata already returned by the configured Lens provider. It never fabricates missing values."
});
