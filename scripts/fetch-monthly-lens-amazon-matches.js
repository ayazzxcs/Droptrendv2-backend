...
const sameMonthCache = existingMeta?.month === month;
...
async function findOneAmazonByLens(imageUrl) {
  ...
  try {
    ...
    console.log(`${name} returned ${links.length} extracted links, ${amazon.length} Amazon links`);
    ...
    if (/429|run out of searches|out of searches|quota|limit|exhausted|credits|insufficient/i.test(message)) {
      ...
      writeJson(USAGE_PATH, usage);
      console.log(`${name} quota exhausted. Switching to next provider.`);
    } else {
...
for (let i = START_INDEX; i < endIndex; i++) {
  ...
  try {
    ...
    console.log(`Amazon URL found via ${found.provider}: ${found.link.url}`);
    console.log(`Scraping Amazon page with Puppeteer: ${found.link.url}`);
    ...
    try {
      ...
      console.log(`Amazon Puppeteer failed for ${name}: ${err.message}`);
    ...
    if (!data.title || !data.rating) {
      console.log(
        `Amazon data rejected: missing title/rating | title="${data.title}" rating="${data.rating}" reviews="${data.ratingsTotal}"`
      );
    ...
    if (data.matchScore < MIN_TITLE_MATCH) {
      console.log(
        `Amazon data rejected: weak title match ${data.matchScore}/${MIN_TITLE_MATCH} | CJ="${name}" | Amazon="${data.title}"`
      );
...
writeJson("lens-amazon-meta.json", {
  ...
  note: "Monthly cache mode: first run of a new month starts fresh; later same-month runs skip existing Amazon data. Uses one provider until its limit is reached, then moves to the next. Provider chain includes Decodo account 1 and Decodo account 2. One Lens request and one Amazon result per product."
});
