...
const rawExistingAmazon = RESET_AMAZON_CACHE ? [] : readJson(AMAZON_PRODUCTS_PATH, []);
...
function amazonSignalKey(signal) {
  const productId = cleanKey(signal.productId || signal.cjProductId || signal.sourceProductId || signal.id);
  if (productId) return `product:${productId}`;

  const productNameValue = cleanKey(signal.productName || signal.cjProductName || signal.originalProductName || signal.keyword);
  if (productNameValue) return `name:${productNameValue}`;

  const productUrlValue = cleanKey(signal.productUrl || signal.url || signal.amazonUrl);
  if (productUrlValue) return `url:${productUrlValue}`;

  const titleValue = cleanKey(signal.title || signal.amazonTitle);
  if (titleValue) return `title:${titleValue}`;

  return "";
}

function amazonQuality(signal) {
  return (
    num(signal.score) * 1000 +
    num(signal.matchScore) * 20 +
    num(signal.bestRating) * 20 +
    Math.log10(num(signal.bestRatingsTotal) + 1) * 60 +
    (signal.productUrl || signal.url || signal.amazonUrl ? 30 : 0) +
    (signal.asin ? 30 : 0) +
    (signal.isBestSeller ? 40 : 0)
  );
}
...
function dedupeAmazonSignals(signals) {
  ...
  for (const signal of Array.isArray(signals) ? signals : []) {
    const key = amazonSignalKey(signal);
    if (!key) continue;
...
function productCacheKeys(product) {
  const out = [];
  const productId = cleanKey(product.id || product.raw?.pid || product.raw?.productId);
  const name = cleanKey(productName(product));

  if (productId) out.push(`product:${productId}`);
...
const existingAmazon = dedupeAmazonSignals(rawExistingAmazon);
const existingAmazonKeys = new Set(existingAmazon.map(amazonSignalKey).filter(Boolean));
const newAmazonMap = new Map();

console.log(
  `Amazon cache mode: ${RESET_AMAZON_CACHE ? "RESET monthly refresh" : "KEEP existing cache"}; ` +
  `existing raw ${rawExistingAmazon.length}; deduped cache ${existingAmazon.length}`
);

function alreadyHasAmazon(product) {
  return productCacheKeys(product).some(key =>
    existingAmazonKeys.has(key) || newAmazonMap.has(key)
  );
}
...
for (let i = START_INDEX; i < endIndex; i++) {
  ...
  try {
    ...
    const cacheKey = productCacheKeys(p)[0] || amazonSignalKey(signal);
    newAmazonMap.set(cacheKey, signal);
    amazonSignals = [...newAmazonMap.values()];
...
writeJson("lens-amazon-meta.json", {
  ...
  month,
  mode: "monthly cache + chunk-only new Amazon signals",
  resetAmazonCache: RESET_AMAZON_CACHE,
  existingRawCacheCount: rawExistingAmazon.length,
  existingDedupedCacheCount: existingAmazon.length,
  newAmazonSignals: amazonSignals.length,
  ...
  note: "Manual runs keep cached Amazon data and output only new missing matches. Monthly reset runs ignore old cache and rebuild fresh data."
});

console.log(`Monthly Lens complete. New matches: ${matches.length}, output signals: ${amazonSignals.length}, failures: ${failures.length}`);
