...
function scoreFromValues(values) {
  ...
  return {
    ...
    trendDirection: growthPercent > 0 ? "up" : growthPercent < 0 ? "down" : "flat"
  };
}
...
async function fetchTrendForVariant(page, variant, originalKeyword) {
  ...
  return {
    ...
    trendDirection: scored.trendDirection,
    match: 1,
    source: "google-trends-network",
    fetchedAt: new Date().toISOString()
  };
...
for (const item of keywords) {
  ...
  try {
    ...
    if (signal?.match) {
      signals.push(signal);
      console.log(`Saved Google trend: ${keyword} -> ${signal.usedKeyword}, score ${signal.score}, growth ${signal.growthPercent}% (${signal.trendDirection || "unknown"}), points ${signal.timelinePoints}`);
    } else {
      failed.push(keyword);
