...
// Strict mode: cache-level + picker-level meaningful overlap check based on FULL product title.
...
// Expanded list of generic words to ignore during overlap calculation.
const OVERLAP_IGNORE_WORDS = new Set([
  ...
  // Added generic adjectives and descriptors
...
async function fetchTrendForVariantCached(page, variant, originalKeyword, originalTitle) {
  ...
  // **Cache-level overlap check using the full product title**
  const titleWords = new Set(cleanKeyword(originalTitle).split(/\s+/).filter(w => !OVERLAP_IGNORE_WORDS.has(w)));
  if (titleWords.size > 0) {
    const variantWords = new Set(cleanKeyword(variant).split(/\s+/).filter(w => !OVERLAP_IGNORE_WORDS.has(w)));
    let overlap = 0;
    ...
    if (overlap === 0) {
      console.log(`Cache-level reject: variant "${variant}" has no meaningful overlap with title "${originalTitle}". Fetching fresh.`);
      VARIANT_RESULT_CACHE.delete(variant);
...
/**
 * Pick the best variant signal with strict meaningful overlap using the FULL product title.
 * Only variants that share at least one non-generic word with the original title are considered.
 * If none exist, return null (product gets no Google score).
 */
function pickBestVariantSignal(signals, originalKeyword, originalTitle) {
  ...
  // If original title has no meaningful words, we can't enforce overlap – pick highest score (rare case).
  if (titleWords.size === 0) {
    matched.sort((a, b) => trendSignalScore(b) - trendSignalScore(a));
    return matched[0];
  }

  // Score each signal by meaningful overlap.
  const scored = matched.map(signal => {
    const signalWords = cleanKeyword(signal.usedKeyword).split(/\s+/).filter(w => !OVERLAP_IGNORE_WORDS.has(w));
    let meaningfulOverlap = 0;
    for (const w of signalWords) {
      if (titleWords.has(w)) meaningfulOverlap += 1;
    }
    return { signal, meaningfulOverlap, score: trendSignalScore(signal) };
  });

  // Only keep candidates with at least 1 meaningful overlap.
  const candidatesWithOverlap = scored.filter(s => s.meaningfulOverlap > 0);

  // If none, return null – this product gets no Google score.
  if (candidatesWithOverlap.length === 0) {
    console.log(`Google Trends rejecting all variants for "${originalTitle}" – no meaningful overlap.`);
    return null;
  }

  // Sort by meaningful overlap (highest first), then by score.
  candidatesWithOverlap.sort((a, b) => b.meaningfulOverlap - a.meaningfulOverlap || b.score - a.score);
...
async function fetchWithVariants(pages, item) {
  ...
  const fullTitle = item?.fullTitle || item?.sourceTitles?.[0] || item?.title || keyword;
  ...
  // Pass the full title for overlap check.
...
writeJson("google-trends-meta.json", {
  ...
  note: "Strict mode: overlap check uses the FULL product title (not just extracted keyword) to prevent false matches like 'cat pets' on a squishy toy.",
