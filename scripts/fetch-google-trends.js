...
function variantMatchesTitle(variant, fullTitle) {
  ...
  const titleWords = new Set(fullTitle.split(/\s+/).filter(Boolean));
  ...
  // 1) Primary: at least one meaningful (non-generic) overlap
  if (meaningfulOverlap > 0) {
    console.log(`[ACCEPT] Meaningful overlap: ${meaningfulOverlap} for "${variant}"`);
    return true;
  }

  // 2) Total overlap >= 3 AND at least one overlapping word has IDF > 1.5
  if (totalOverlap >= 3) {
    const highIdfWords = [...variantWords].filter(w => (TITLE_IDF.get(w) || 0) > 1.5);
    const overlapHighIdf = highIdfWords.some(w => titleWords.has(w));
    if (overlapHighIdf) {
      console.log(`[ACCEPT] Total overlap ${totalOverlap} with high-IDF word for "${variant}"`);
      return true;
    }
  }

  // 3) Total overlap >= 2 AND weighted overlap >= 0.4
  if (totalOverlap >= 2) {
    const wOverlap = weightedOverlap(variantWords, titleWords);
    if (wOverlap >= 0.4) {
      console.log(`[ACCEPT] Weighted overlap ${wOverlap.toFixed(3)} for "${variant}"`);
      return true;
    }
  }

  console.log(`[REJECT] "${variant}" vs "${fullTitle}" – total=${totalOverlap}, meaningful=${meaningfulOverlap}`);
  return false;
}
...
function pickBestVariantSignal(signals, originalKeyword, originalTitle) {
  ...
  // Score by meaningful overlap, total overlap, then score
...
async function fetchWithVariants(pages, item) {
  ...
  const bestSignal = pickBestVariantSignal(checkedSignals, keyword, cleanTitle);
...
writeJson("google-trends-meta.json", {
  ...
  note: "Hybrid + IDF-weighted overlap. Uses full product titles. No Jaro-Winkler fallback.",
