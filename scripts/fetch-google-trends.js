...
// Strict mode: cache-level + picker-level meaningful overlap check based on product-specific title.
...
const OVERLAP_IGNORE_WORDS = new Set([
  ...
  "zircon", "drop", "earrings", "kitty", "pets"
]);
...
async function fetchTrendForVariantCached(page, variant, originalKeyword, originalTitle) {
  ...
  // Cache-level overlap check using the product-specific title
...
async function fetchWithVariants(pages, item) {
  ...
  // Use product-specific title if available, otherwise fallback
...
writeJson("google-trends-meta.json", {
  ...
  note: "Strict mode: overlap check uses product-specific title (productTitle). Expanded ignore list includes light, luxury, sense, retro, geometric, studs, mid, century, zircon, drop, earrings, kitty, pets.",
