...
/**
 * Pick the best variant signal, but prefer variants that share words with the original keyword.
 * This prevents unrelated cached variants (like "office cup") from being attached to a product
 * that has no relation (like "slippers").
 */
function pickBestVariantSignal(signals, originalKeyword) {
  ...
  matched.sort((a, b) => {
    const aWords = new Set(cleanKeyword(a.usedKeyword).split(/\s+/).filter(Boolean));
    const bWords = new Set(cleanKeyword(b.usedKeyword).split(/\s+/).filter(Boolean));
    const aOverlap = [...aWords].filter(w => originalWords.has(w)).length;
    const bOverlap = [...bWords].filter(w => originalWords.has(w)).length;

    // Sort by overlap (higher is better), then by score, then by word count.
...
writeJson("google-trends-meta.json", {
  ...
  note: "Captures Google Trends internal timeline JSON from Explore page. Shared variants are fetched once per matrix job and reused across product anchors, preserving unlimited generated variant coverage while avoiding duplicate network requests. The picker now prefers variants that share words with the original product keyword, preventing unrelated cache hits.",
