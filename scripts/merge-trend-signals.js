...
function keywordMatchScore(product, signalKeyword) {
  ...
  if (pKeywords.includes(sig)) {
    return 85 + Math.min(15, sigWords.length * 4);
  }

  if (text.includes(sig)) {
    return 60 + Math.min(25, sigWords.length * 7);
  }

  const pTokens = new Set(tokenizeProductText(text));
  const hits = sigWords.filter(t => pTokens.has(t));

  if (sigWords.length === 1) return hits.length ? 20 : 0;
...
function findBestSignal(product, signals) {
  const matches = signals
    .map(s => ({
      signal: s,
      matchScore: keywordMatchScore(product, s.keyword),
...
function findGoogle(p) {
  return findBestSignal(p, googleSignals);
}

function findAmazon(p) {
  return findBestSignal(p, amazonProducts);
}
...
console.log(`Merged ${googleSignals.length} Google signals and ${new Set(amazonProducts.map(a => a.keyword)).size} Amazon signals into ${merged.length} products with AI-style keyword extraction.`);
