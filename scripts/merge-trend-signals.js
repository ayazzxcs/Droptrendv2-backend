...
function findGoogleMatches(p) {
  ...
  const matches = googleSignals
    .map(signal => {
      ...
      let matchScore = exactProductMatch ? 200 : 0;
      let matchedKeyword = "";
...
console.log(`Merged ${googleSignals.length} Google signals and ${amazonSignals.length} deduped monthly Amazon signals from ${rawAmazonSignals.length} raw Amazon records into ${merged.length} products with combined multi-keyword Google Trends scoring and source-aware supplier scoring. Updated trend-history.json for ${Object.keys(nextHistory).length} products.`);
