...
function findGoogleMatches(p) {
  const keywordPool = extractProductKeywords(p, 8);
...
const merged = products.map(p => {
  ...
  return {
    ...
    aiKeywords: p.aiKeywords || extractProductKeywords(p, 8),
    specificKeywords: p.specificKeywords || extractProductKeywords(p, 8),
    trendProof: {
      ...
      googleTrends: googleCombined.best ? {
        ...
        checkedKeywords: extractProductKeywords(p, 8)
      } : null,
      amazon: a ? {
        keyword: a.keyword,
        score: amazonScore,
...
console.log(`Merged ${googleSignals.length} Google signals and ${new Set(amazonProducts.map(a => a.keyword)).size} Amazon signals into ${merged.length} products with combined multi-keyword Google Trends scoring.`);
