...
async function main() {
  ...
  // IMPORTANT: DropTrend now pulls CJ TRENDING products only.
  // We do NOT fall back to the normal/full CJ product list here, because the site
  // should not be filled with ordinary CJ products when trending data is unavailable.
  const trendingPages = Number(process.env.CJ_TRENDING_PAGES || MAX_PAGES || 30);
  ...
  if (!trendingItems.length) {
    ...
    fs.writeFileSync("products-meta.json", JSON.stringify({
      ...
      mode: "CJ trending products only",
      trendingOnly: true,
      error: "CJ trending endpoint returned no products. Normal product list was intentionally not used."
    }, null, 2));
    throw new Error("CJ trending endpoint returned no products. Keeping output empty because CJ_TRENDING_ONLY is enabled.");
  }

  const all = [];
  const seen = new Set();
  let sampleRaw = trendingItems[0] || null;

  for (const item of trendingItems) {
    const normalized = normalizeProduct(item);
    normalized.tags = ["CJ trending", "CJ product"];
    normalized.cjTrending = true;
  ...
  fs.writeFileSync("products-meta.json", JSON.stringify({
    updatedAt: new Date().toISOString(),
    count: all.length,
    maxPages: trendingPages,
    pageSize: PAGE_SIZE,
    source: "CJdropshipping",
    mode: "CJ trending products only + Google Trends and Amazon trend scoring after this step",
    trendingOnly: true,
    trendingCount: trendingItems.length,
    note: "Normal CJ product list is intentionally disabled. Only CJ trending/searchType=2 products are saved to products.json."
  }, null, 2));
  ...
  console.log(`Saved ${all.length} CJ trending products only to products.json`);
}
