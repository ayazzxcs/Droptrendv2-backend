...
function sanitizeDiscoveredKeyword(value) {
  let k = normalizeKeyword(decodeValue(value || ""))
    .replace(/\b(ali\s*express|aliexpress|choice|super deals?|flash deals?|free shipping|coupon|coupons|shop now|view all|see all|more to love|limited offer|new arrivals?|best sellers?|top ranking|recommended|sponsored|store|official store|sale|deals|deal|products?|categories?|category|wholesale|homepage|buyer protection|download app|app only|cart|wishlist|sign in|account|orders?|help center)\b/g, " ")
  ...
  k = k.replace(/\b(for|with|and|the|a|an|to|of|in|on|at|by|from)\b/g, " ").replace(/\s+/g, " ").trim();

  const words = k.split(/\s+/).filter(Boolean);
  if (!words.length || words.length > 4) return "";
  if (k.length < 3 || k.length > 55) return "";
  if (/^\d+$/.test(k)) return "";
  if (/^(home|men|women|kids|baby|pet|car|phone|fashion|beauty|tools|accessories|gadget|gadgets|decor|toy|toys|dress|light|lights|organizer)$/.test(k)) return "";
...
async function discoverAliExpressKeywordPool(browser) {
  ...
  try {
    ...
    const discoveryUrls = [
      ...
      "https://www.aliexpress.com/category.html",
