...
const RESET_SESSION_AFTER_KEYWORD = String(process.env.ALIEXPRESS_RESET_SESSION_AFTER_KEYWORD || "true").toLowerCase() !== "false";
...
async function extractCards(page) {
  return await page.evaluate(() => {
    ...
    const extractEmbeddedProducts = () => {
      // Stable mode: use only visible DOM product cards.
      // Embedded AliExpress hydration parsing caused page.evaluate ReferenceError in GitHub Actions.
      return [];
      const embedded = [];
      const texts = [];
...
async function createAliExpressPage(browser) {
  ...
  if (typeof page.emulateTimezone === "function") {
    await page.emulateTimezone("America/New_York").catch(() => null);
  }
...
async function closeSharedContext(shared) {
  if (shared?.page && !shared.page.isClosed()) {
    try { await shared.page.close(); } catch {}
  }
...
async function ensureSharedPage(browser, shared) {
  ...
  const created = await createAliExpressPage(browser);
  shared.context = created.context;
  shared.page = created.page;
  return shared.page;
}
...
async function main() {
  ...
  try {
    ...
    for (const item of keywordPool) {
      ...
      // Do not let one AliExpress search session poison the next keyword.
      // If AliExpress returns a good page for cat toy and then an empty page for neck fan,
      // the next keyword now starts from a clean context instead of reusing that possibly bad session.
      if (RESET_SESSION_AFTER_KEYWORD) {
        await closeSharedContext(shared);
      }
  ...
  writeJson(META_PATH, {
    ...
    source: "AliExpress",
    mode: "Puppeteer keyword/category search scraper",
    count: output.length,
