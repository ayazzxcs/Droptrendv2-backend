...
async function fetchKeywordProducts(page, keyword, attempt = 1) {
  ...
  for (let i = 0; i < 5; i++) {
    await page.mouse.wheel(0, 1600 + Math.floor(Math.random() * 600));
...
async function main() {
  ...
  writeJson(META_PATH, {
    ...
    note: "AliExpress is treated as a product source like CJ: it fetches high-demand AliExpress products using only AliExpress/dropshipping seed categories. Normal searches use one warm AliExpress browser session for stable first-page product counts. Captcha-blocked keywords rotate into a fresh browser context with cooldown instead of being skipped immediately. CJ product titles and Google Trends keywords are intentionally not used as AliExpress keywords. If AliExpress blocks or returns no data, old aliexpress-products.json cache is kept."
  });
