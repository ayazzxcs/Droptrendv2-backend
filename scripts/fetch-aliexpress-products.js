...
async function enrichAliExpressProductDetails(products) {
  ...
  try {
    detailSession = await createAliExpressPage(browser);
    const page = detailSession.page;
    ...
    await closeContext(detailSession);
...
async function main() {
  ...
  if (finalProducts.length) {
    try {
      ...
      // Detail enrichment is optional metadata. Keep the newly collected listing
      // products instead of discarding the whole run when the detail browser fails.
      console.log(`AliExpress detail enrichment failed globally; keeping listing data: ${err.message}`);
    }
  }
