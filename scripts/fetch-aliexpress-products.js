...
async function fetchKeywordProductsInFreshContext(browser, keyword, attempt = 1) {
  const session = await createAliExpressPage(browser);
  session.collector = attachNetworkCollector(session.page);

  try {
    return await fetchKeywordProducts(session.page, session.collector, keyword, attempt);
  } finally {
...
async function fetchKeywordProductsWithRetries(browser, keyword) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      const products = await fetchKeywordProductsInFreshContext(browser, keyword, attempt);
...
async function main() {
  ...
  writeJson(META_PATH, {
    ...
    source: "AliExpress",
    mode: "Puppeteer page-1 DOM + network + HTML extractor",
    count: output.length,
    ...
    note: "AliExpress is treated as a product source like CJ. It fetches page 1 sorted by orders/sales for AliExpress/dropshipping seed categories. This version uses Puppeteer with fresh per-keyword contexts, tall viewport, deeper scrolling, visible DOM extraction, network-response extraction, and safe HTML hydration extraction. It keeps products with title + URL + image even if price is hidden, so Amazon Lens still has usable product images. Zero/low-count pages are retried in fresh sessions and are not accepted as normal. CJ product titles and Google Trends keywords are intentionally not used as AliExpress keywords. If AliExpress blocks or returns no data, old aliexpress-products.json cache is kept."
  });
