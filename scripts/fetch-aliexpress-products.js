...
const previousTrendKeywords = readJson("trend-keywords.json", []);
...
function buildKeywordPool() {
  ...
  // Do not use CJ product titles as AliExpress keywords.
  // AliExpress should behave as its own product source, using previous
  // Google Trends winners plus broad AliExpress/dropshipping seed categories.
  for (const item of SEED_KEYWORDS) add(item, 3);

  const trendList = Array.isArray(previousTrendKeywords) ? previousTrendKeywords : [];
  for (const item of trendList.slice(0, 300)) {
    if (typeof item === "string") add(item, 5);
    else add(item.keyword || item.usedKeyword || item.q, 5);
  }
...
async function main() {
  ...
  writeJson(META_PATH, {
    ...
    note: "AliExpress is treated as a product source like CJ: it fetches high-demand AliExpress products using previous Google Trends keywords plus AliExpress/dropshipping seed categories. CJ product titles are intentionally not used as AliExpress keywords. If AliExpress blocks or returns no data, old aliexpress-products.json cache is kept."
  });
