...
async function extractCards(page) {
  return await page.evaluate(() => {
    ...
    try {
      ...
      console.log("AliExpress embedded extraction skipped:", err?.message || String(err));
    }
...
async function fetchKeywordProducts(page, keyword, attempt = 1) {
  ...
  for (const url of makeAliExpressUrlVariants(keyword, 1)) {
    ...
    console.log(`AliExpress raw extracted cards for ${keyword}: ${cards.length}`);
    ...
    console.log(`AliExpress low product count for ${keyword}: ${products.length}. Trying alternate page URL...`);
...
async function fetchKeywordProductsWithRetries(browser, keyword, shared) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (products.length > 0 && products.length < MIN_PRODUCTS_PER_KEYWORD && attempt < MAX_RETRIES) {
        throw new Error(`AliExpress low product count for keyword: ${keyword}: ${products.length}`);
      }
