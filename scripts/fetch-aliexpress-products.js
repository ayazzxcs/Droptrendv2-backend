...
const DETAIL_ENRICH_LIMIT = Number(process.env.ALIEXPRESS_DETAIL_ENRICH_LIMIT || 250);
...
async function enrichAliExpressProductDetails(products) {
  ...
  const candidates = list
    ...
    .slice(0, Math.max(0, DETAIL_ENRICH_LIMIT));

  if (!candidates.length) return list;
  console.log(`AliExpress detail enrichment enabled for ${candidates.length}/${list.length} products.`);
