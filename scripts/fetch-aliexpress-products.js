...
async function probeAliExpressKeyword(browser, state, keyword) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    console.log(`AliExpress keyword probe search: ${keyword}${attempt > 1 ? ` (attempt ${attempt})` : ""}`);
    ...
    try {
      ...
      if (!isEmpty || attempt >= MAX_RETRIES) {
      ...
      console.log(`AliExpress keyword probe attempt failed: ${keyword}: ${err.message}`);

      // Probe recovery now follows product-fetch recovery:
      // same keyword, fresh AliExpress session, same AliExpress URL variants.
      await resetAliExpressSession(browser, state, captcha ? `captcha probe on ${keyword}` : `empty/blocked probe on ${keyword}`);
    ...
    for (const candidate of candidateList) {
      ...
      try {
        ...
        if (isEmptyProbe) {
          ...
          // Do not stop the discovery tail just because the current session got empty/blocked.
          // probeAliExpressKeyword already retried the same keyword with a fresh session.
          // Keep moving through the AliExpress-discovered candidate list.
        ...
        if (isCaptchaError(err)) {
          emptyProbeStreak += 1;
          totalEmptyProbes += 1;
          // Keep checking later AliExpress-discovered keywords; the probe already reset/retried.
        }
      }
      await sleep(Math.max(900, Math.floor(DELAY_MS * 0.6)) + Math.floor(Math.random() * 800));
    }
