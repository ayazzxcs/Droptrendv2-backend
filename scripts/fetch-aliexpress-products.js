...
const STATIC_FALLBACK_ON_DISCOVERY_FAIL =
  String(process.env.ALIEXPRESS_STATIC_FALLBACK_ON_DISCOVERY_FAIL || "false").toLowerCase() === "true";

// Discovery probing is two-phase:
// 1) First pass checks each AliExpress-discovered keyword once without resetting the browser on zero results.
// 2) Zero-result probes are retried after the useful/successful probes have been collected.
// This avoids wasting resets on keywords that may work later, while still recovering if AliExpress soft-blocks.
const DISCOVERY_IMMEDIATE_PROBE_RETRIES = Number(process.env.ALIEXPRESS_DISCOVERY_IMMEDIATE_PROBE_RETRIES || 1);
const DISCOVERY_DEFERRED_PROBE_RETRIES = Number(process.env.ALIEXPRESS_DISCOVERY_DEFERRED_PROBE_RETRIES || MAX_RETRIES);
...
// Product fetching uses the same two-pass recovery idea as discovery probing:
// 1) First pass tries each good keyword once, without resetting the browser on every empty keyword.
// 2) Empty/blocked product keywords are retried after the successful keywords have been fetched.
// This avoids wasting time on reset loops while still giving blocked/empty keywords a second chance.
const PRODUCT_IMMEDIATE_FETCH_RETRIES = Number(process.env.ALIEXPRESS_PRODUCT_IMMEDIATE_FETCH_RETRIES || 1);
const PRODUCT_DEFERRED_FETCH_RETRIES = Number(process.env.ALIEXPRESS_PRODUCT_DEFERRED_FETCH_RETRIES || MAX_RETRIES);
...
function makeKeywordSearchVariants(keyword) {
  // IMPORTANT: Keep exact AliExpress-discovered keyword only.
  // Do not rewrite "summer dress" into "dress" / "summer" / title fallback variants.
  const exact = sanitizeDiscoveredKeyword(keyword);
  return exact ? [exact] : [];
}
...
async function probeAliExpressKeyword(browser, state, keyword, options = {}) {
  const maxAttempts = Math.max(1, Number(options.maxAttempts || MAX_RETRIES));
  const resetOnEmpty = options.resetOnEmpty !== false;
  const resetEmptyStreakLimit = Math.max(1, Number(options.resetEmptyStreakLimit || DISCOVERY_PROBE_EMPTY_RESET_STREAK));
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const session = await getAliExpressSession(browser, state);
    console.log(`AliExpress keyword probe search${label}: ${keyword}${attempt > 1 ? ` (attempt ${attempt})` : ""}`);

    try {
      for (const url of makeAliExpressUrlVariants(keyword, 1).slice(0, 2)) {
        const networkMark = session.collector.mark();
        await session.page.goto(url, { waitUntil: "domcontentloaded", timeout: SEARCH_TIMEOUT_MS });
        await waitForNetworkIdleSafe(session.page, 15000);
        ...
        if (isCaptchaText(bodyText)) {
          throw new Error(`AliExpress captcha/security check for probe keyword: ${keyword}`);
        }

        await fullPageScroll(session.page);
        await waitForNetworkIdleSafe(session.page, 10000);

        const domCards = await extractDomCards(session.page).catch(() => []);
        const networkCards = session.collector.since(networkMark);
        ...
        if (products.length >= DISCOVERY_MIN_PRODUCTS || rawCards.length >= DISCOVERY_MIN_PRODUCTS) {
          state.emptyStreak = 0;
          state.probeEmptyAttemptStreak = 0;
          return best;
        }
      }

      const isEmpty = best.products.length === 0 && best.rawCount === 0 && best.dom === 0 && best.network === 0 && best.html === 0;
      if (!isEmpty || attempt >= maxAttempts) {
        state.emptyStreak = isEmpty ? (state.emptyStreak || 0) + 1 : 0;
        if (!isEmpty) state.probeEmptyAttemptStreak = 0;
        return best;
      }

      throw new Error(`AliExpress empty probe page for keyword: ${keyword}`);
    } catch (err) {
      lastError = err;
      const captcha = isCaptchaError(err);
      console.log(`AliExpress keyword probe attempt failed${label}: ${keyword}: ${err.message}`);

      if (captcha) {
        await resetAliExpressSession(browser, state, `captcha probe on ${keyword}`);
        localEmptyAttempts = 0;
        state.probeEmptyAttemptStreak = 0;
        if (attempt < maxAttempts) {
          console.log(`AliExpress probe captcha retry queued for same keyword: ${keyword}. Cooling down ${CAPTCHA_COOLDOWN_MS}ms`);
          await sleep(CAPTCHA_COOLDOWN_MS);
        }
        continue;
      }

      localEmptyAttempts += 1;
      state.probeEmptyAttemptStreak = (state.probeEmptyAttemptStreak || 0) + 1;

      if (attempt < maxAttempts) {
        if (resetOnEmpty && (localEmptyAttempts >= resetEmptyStreakLimit || state.probeEmptyAttemptStreak >= resetEmptyStreakLimit)) {
          await resetAliExpressSession(browser, state, `repeated empty/blocked probes near ${keyword}`);
          localEmptyAttempts = 0;
          state.probeEmptyAttemptStreak = 0;
        } else {
          console.log(
            `AliExpress probe keeping same browser session after empty result: ${keyword} ` +
            `(empty attempt streak ${state.probeEmptyAttemptStreak}/${resetEmptyStreakLimit})`
          );
        }
  ...
  if (best.rawCount > 0 || best.products.length > 0) return best;
  throw lastError || new Error(`AliExpress probe failed for keyword: ${keyword}`);
}
...
async function discoverAliExpressKeywordPool(browser) {
  ...
  const discoveryState = { session: null, emptyStreak: 0, probeEmptyAttemptStreak: 0 };
  ...
  try {
    ...
    for (const url of discoveryUrls) {
      try {
        ...
        const html = await session.page.content().catch(() => "");
        const pageProducts = extractProductsFromText(html, url);
    ...
    const deferredEmptyProbes = [];
    let checked = 0;
    let deferredChecked = 0;

    function recordProbeResult(candidate, result, phase = "") {
      ...
      console.log(
        `AliExpress keyword probe${phase}: ${candidate.keyword}: usable=${result.products.length}, raw=${result.rawCount}, ` +
        `dom=${result.dom}, network=${result.network}, html=${result.html}, score=${score}`
      );

      if (isEmptyProbe) return "empty";

      if (result.products.length >= DISCOVERY_MIN_PRODUCTS || result.rawCount >= DISCOVERY_MIN_PRODUCTS) {
        scored.push({
          keyword: candidate.keyword,
          weight: score,
          source: phase ? "aliexpress-discovered-validated-deferred" : "aliexpress-discovered-validated",
      ...
      console.log(
        `AliExpress keyword probe rejected weak candidate${phase}: ${candidate.keyword} ` +
    ...
    for (const candidate of candidateList) {
      ...
      checked += 1;
      try {
        const result = await probeAliExpressKeyword(browser, discoveryState, candidate.keyword, {
          maxAttempts: DISCOVERY_IMMEDIATE_PROBE_RETRIES,
          resetOnEmpty: false,
          label: "first-pass"
        });

        const status = recordProbeResult(candidate, result);
        if (status === "empty") {
          deferredEmptyProbes.push(candidate);
          console.log(
            `AliExpress keyword probe deferred empty candidate for later retry: ${candidate.keyword} ` +
            `(deferred ${deferredEmptyProbes.length})`
          );
        }
      } catch (err) {
        console.log(`AliExpress keyword probe first-pass failed: ${candidate.keyword}: ${err.message}`);
        if (isCaptchaError(err)) {
          deferredEmptyProbes.push(candidate);
          console.log(`AliExpress keyword probe deferred captcha/blocked candidate for later retry: ${candidate.keyword}`);
        }
      }
      await sleep(Math.max(900, Math.floor(DELAY_MS * 0.6)) + Math.floor(Math.random() * 800));
    }

    if (deferredEmptyProbes.length) {
      console.log(
        `AliExpress retrying ${Math.min(deferredEmptyProbes.length, DISCOVERY_DEFERRED_RETRY_LIMIT)} deferred empty probe keywords ` +
        `after first-pass successful probes. Browser session will reset only after ${DISCOVERY_PROBE_EMPTY_RESET_STREAK} repeated empty probe attempts.`
      );

      for (const candidate of deferredEmptyProbes.slice(0, DISCOVERY_DEFERRED_RETRY_LIMIT)) {
        deferredChecked += 1;
        try {
          const result = await probeAliExpressKeyword(browser, discoveryState, candidate.keyword, {
            maxAttempts: DISCOVERY_DEFERRED_PROBE_RETRIES,
            resetOnEmpty: true,
            resetEmptyStreakLimit: DISCOVERY_PROBE_EMPTY_RESET_STREAK,
            label: "deferred-retry"
          });

          const status = recordProbeResult(candidate, result, " deferred-retry");
          if (status === "empty") {
            console.log(`AliExpress deferred probe still empty after retry: ${candidate.keyword}`);
          }
        } catch (err) {
          console.log(`AliExpress deferred keyword probe failed: ${candidate.keyword}: ${err.message}`);
        }
        await sleep(Math.max(900, Math.floor(DELAY_MS * 0.7)) + Math.floor(Math.random() * 900));
      }
    }
    ...
    if (finalKeywords.length) {
      console.log(
        `AliExpress final keyword pool from AliExpress discovery: ${finalKeywords.length} searches ` +
        `(first-pass checked ${checked}, deferred retried ${deferredChecked})`
      );
...
async function fetchKeywordProducts(page, collector, keyword, attempt = 1) {
  ...
  for (const searchKeyword of searchVariants) {
    for (const url of makeAliExpressUrlVariants(searchKeyword, 1)) {
      ...
      console.log(
        `AliExpress extracted for ${keyword}: ` +
      ...
      console.log(
        `AliExpress low usable count for ${keyword}: ` +
        `${products.length} usable from ${rawCards.length} raw. Trying alternate AliExpress URL with same keyword...`
      );
...
function isRetryableAliExpressFetchError(err) {
  const message = String(err?.message || err || "");
  return /captcha|security check|blocked|access denied|low\/empty|empty product page|low product count|timeout|navigation timeout|net::|target closed|protocol error/i.test(message);
}
...
async function fetchKeywordProductsWithRetries(browser, state, keyword, options = {}) {
  ...
  const resetOnEmpty = options.resetOnEmpty !== false;
  const phaseLabel = options.phaseLabel ? ` ${options.phaseLabel}` : "";
  let lastError = null;
  let bestProducts = [];
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (products.length >= MIN_PRODUCTS_PER_KEYWORD || products.length >= PRODUCTS_PER_KEYWORD) {
        state.emptyStreak = 0;
        state.productEmptyStreak = 0;
        return products;
      }

      // Keep partial valid products, but let the deferred pass retry it later if this is the first pass.
      if (products.length > 0 && attempt >= Math.min(2, maxRetries)) {
        console.log(`AliExpress accepting partial usable count for ${keyword}${phaseLabel}: ${products.length} products`);
        state.emptyStreak = 0;
        state.productEmptyStreak = 0;
        return products;
      }
      ...
      lastError = err;
      const captcha = isCaptchaError(err);
      const retryable = isRetryableAliExpressFetchError(err);
      console.log(`AliExpress attempt failed${phaseLabel}: ${keyword}: ${err.message}`);

      if (retryable) {
        state.productEmptyStreak = (state.productEmptyStreak || 0) + 1;
      }

      const shouldResetNow =
        captcha ||
        RESET_SESSION_AFTER_KEYWORD ||
        (resetOnEmpty && retryable && state.productEmptyStreak >= PRODUCT_EMPTY_RESET_STREAK);

      if (shouldResetNow) {
        await resetAliExpressSession(
          browser,
          state,
          captcha ? `captcha on ${keyword}` : `repeated empty product fetches (${state.productEmptyStreak}/${PRODUCT_EMPTY_RESET_STREAK})`
        );
        state.productEmptyStreak = 0;
      } else if (retryable) {
        console.log(
          `AliExpress product fetch deferred session reset: ${keyword} ` +
          `(empty streak ${state.productEmptyStreak}/${PRODUCT_EMPTY_RESET_STREAK})`
        );
      }
  ...
  if (bestProducts.length) {
    console.log(`AliExpress using best partial result for ${keyword}${phaseLabel}: ${bestProducts.length} products`);
    return bestProducts;
  }
...
async function main() {
  ...
  const all = [];
  const errors = [];
  const deferredProductRetry = [];

  const sessionState = { session: null, emptyStreak: 0, productEmptyStreak: 0 };
  ...
  try {
    ...
    for (const item of keywordPool) {
      ...
      try {
        ...
        console.log(`AliExpress keyword deferred for later product retry: ${item.keyword}: ${err.message}`);
        if (RETRY_FAILED_KEYWORDS && isRetryableAliExpressFetchError(err)) {
          deferredProductRetry.push({ ...item, firstError: err.message });
        } else {
    ...
    if (RETRY_FAILED_KEYWORDS && all.length < MAX_PRODUCTS && deferredProductRetry.length) {
      const retryItems = deferredProductRetry.slice(0, PRODUCT_DEFERRED_RETRY_LIMIT);
      console.log(
        `Retrying ${retryItems.length}/${deferredProductRetry.length} deferred AliExpress product keywords ` +
        `after first-pass successful keywords have been fetched.`
      );

      await resetAliExpressSession(browser, sessionState, "AliExpress product deferred retry pass");
      await sleep(Math.min(CAPTCHA_COOLDOWN_MS, 30000));
      ...
      for (const item of retryItems) {
        ...
        try {
          const products = await fetchKeywordProductsWithRetries(browser, sessionState, item.keyword, {
            ...
            resetOnEmpty: true,
            phaseLabel: "deferred-retry"
          });
          console.log(`AliExpress deferred product retry ${item.keyword}: ${products.length} products`);
          all.push(...products);
          const deduped = dedupeProducts(all);
          all.length = 0;
          all.push(...deduped);
          ...
          console.log(`AliExpress deferred product retry failed: ${item.keyword}: ${err.message}`);
          errors.push({ keyword: item.keyword, firstError: item.firstError, error: `deferred product retry: ${err.message}` });
        }
  ...
  writeJson(META_PATH, {
    ...
    usedFallback,
    deferredProductRetryCount: deferredProductRetry.length,
    errorCount: errors.length,
    errors: errors.slice(0, 80),
    note: "AliExpress is treated as a product source like CJ. The keyword/category pool is discovered from AliExpress itself: homepage/category links, AliExpress product-title clusters, and validation pages sorted by orders/sales. It then fetches page 1 sorted by orders/sales for the strongest validated AliExpress-discovered keywords/categories. This version uses Puppeteer with a persistent warm AliExpress session, no scheduled batch reset, session reset only on zero/captcha/low-count pages, tall viewport, deeper scrolling, visible DOM extraction, network-response extraction, and safe HTML hydration extraction. It keeps products with title + URL + image even if price is hidden, so Amazon Lens still has usable product images. Discovery probing uses a two-pass recovery model: first pass checks AliExpress-discovered keywords without resetting on each zero result; zero-result probes are deferred and retried after the successful probes, resetting the browser session only after repeated empty probe attempts. Product fetching now uses the same two-pass recovery model: first pass tries validated keywords without resetting for every empty keyword, then retries failed/empty product keywords after successful keywords have been fetched, resetting only after repeated empty fetches. It does not rewrite keywords into fallback query variants. CJ product titles and Google Trends keywords are intentionally not used as AliExpress keywords. Emergency static fallback is disabled by default and only runs if ALIEXPRESS_STATIC_FALLBACK_ON_DISCOVERY_FAIL=true."
  });
