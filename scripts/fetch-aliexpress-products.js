...
const USE_DEFAULT_CONTEXT = String(process.env.ALIEXPRESS_USE_DEFAULT_CONTEXT || "true").toLowerCase() !== "false";
const WARMUP_SEARCH = String(process.env.ALIEXPRESS_WARMUP_SEARCH || "true").toLowerCase() !== "false";
...
async function humanWarmActions(page) {
  try {
    await page.mouse.move(250 + Math.random() * 500, 250 + Math.random() * 500, { steps: 8 });
    await sleep(400 + Math.floor(Math.random() * 600));
    await page.mouse.move(700 + Math.random() * 500, 500 + Math.random() * 500, { steps: 10 });
...
async function createBrowserContext(browser) {
  if (USE_DEFAULT_CONTEXT && typeof browser.defaultBrowserContext === "function") {
    return { context: browser.defaultBrowserContext(), usesDefaultContext: true };
  }
  if (typeof browser.createBrowserContext === "function") return { context: await browser.createBrowserContext(), usesDefaultContext: false };
  if (typeof browser.createIncognitoBrowserContext === "function") return { context: await browser.createIncognitoBrowserContext(), usesDefaultContext: false };
  return { context: browser.defaultBrowserContext(), usesDefaultContext: true };
}
...
async function createAliExpressPage(browser) {
  ...
  return { context, page, usesDefaultContext: ctx.usesDefaultContext };
}
...
async function closeContext(obj) {
  ...
  if (obj.context && !obj.usesDefaultContext) {
...
async function createAliExpressSession(browser) {
  ...
  try {
    ...
    await humanWarmActions(session.page);
    await sleep(3000 + Math.floor(Math.random() * 2200));

    if (WARMUP_SEARCH) {
      const warmUrl = makeAliExpressUrl("phone accessories", 1);
      await session.page.goto(warmUrl, { waitUntil: "domcontentloaded", timeout: SEARCH_TIMEOUT_MS });
      await waitForNetworkIdleSafe(session.page, 12000);
      await humanWarmActions(session.page);
...
async function main() {
  ...
  writeJson(META_PATH, {
    ...
    source: "AliExpress",
    mode: "Puppeteer stealth anti-detect persistent-session page-1 DOM + network + HTML extractor",
    count: output.length,
    ...
    note: "AliExpress is treated as a product source like CJ. It fetches page 1 sorted by orders/sales for AliExpress/dropshipping seed categories. This version uses Puppeteer Extra Stealth with anti-detect browser behavior, a persistent warm default browser context, AliExpress homepage/search warmup, real user-agent/language/timezone/viewport, human-like mouse/scroll delays, automatic session reset only on zero/captcha/low-count pages, hard browser restart for bad sessions, tall viewport, deeper scrolling, visible DOM extraction, network-response extraction, and safe HTML hydration extraction. It keeps products with title + URL + image even if price is hidden, so Amazon Lens still has usable product images. Zero/low-count pages are retried in fresh sessions, and if the session remains empty the whole Chrome browser is relaunched once for that keyword before moving on. CJ product titles and Google Trends keywords are intentionally not used as AliExpress keywords. If AliExpress blocks or returns no data, old aliexpress-products.json cache is kept."
  });
